...
func TestFilterDockerInspect_Short(t *testing.T) {
	...
	// Redaction might add spaces/newlines via indenting even if not redacting anything,
	// or it might just return the same if it was already compact.
	// Our new implementation uses json.MarshalIndent for small JSON.
	if !strings.Contains(got, `"Id": "abc"`) {
		t.Errorf("short input should contain Id: abc, got %q", got)
	}
}
