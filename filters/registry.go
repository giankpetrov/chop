...
func init() {
	...
	registerRouter(getSnykFilter, []string{"test", "code"}, "snyk")
	registerRouter(getTrivyFilter, []string{"image", "fs", "repo", "config"}, "trivy")
	registerRouter(getPoetryFilter, []string{"install", "update", "add", "show"}, "poetry")
	registerRouter(getCondaFilter, []string{"install", "update", "create", "list"}, "conda")
	registerFilter(filterPipenv, nil, "pipenv")
	registerFilter(filterGolangciLint, nil, "golangci-lint")
	registerFilter(filterTurbo, nil, "turbo")
	registerRouter(getKustomizeFilter, []string{"build", "diff"}, "kustomize")
	registerRouter(getArgoCDFilter, []string{"app"}, "argocd")
	registerRouter(getFluxFilter, []string{"get", "reconcile"}, "flux")
	registerFilter(filterStern, nil, "stern")
	registerRouter(getVaultFilter, []string{"read", "kv", "list", "secrets", "auth"}, "vault")
	registerFilter(filterDbQuery, nil, "psql", "mysql", "sqlite3")
	registerRouter(getGlabFilter, []string{"mr", "issue", "ci", "pipeline"}, "glab")
